...
def val_one_epoch_lidc(
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            ...
            pred = torch.argmax(torch.softmax(out, dim=1), dim=1, keepdim=True).float()
...
def val_one_epoch_(test_datasets,
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(tqdm(test_loader)):
            ...
            metric_i = test_single_volume(img, msk, model, classes=config.model_config['num_classes'],
